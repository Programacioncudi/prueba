/**
 * Archivo: src/modules/jefaturas/jefaturas.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para jefaturas.
 */
import service from "./jefaturas.service.js";
import { createJefaturasSchema, updateJefaturasSchema } from "./jefaturas.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listJefaturas = async (req, res) => {
  try {
...
export const getJefaturasById = async (req, res) => {
  try {
...
export const createJefaturas = async (req, res) => {
  try {
    const { error, value } = createJefaturasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateJefaturas = async (req, res) => {
  try {
    const { error, value } = updateJefaturasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteJefaturas = async (req, res) => {
  try {
    const ctx = getRequestContext();
