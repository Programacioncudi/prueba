/**
 * Archivo: src/modules/rbac/rbac.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para rbac.
 */
import service from "./rbac.service.js";
import { createRbacSchema, updateRbacSchema } from "./rbac.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listRbac = async (req, res) => {
  try {
...
export const getRbacById = async (req, res) => {
  try {
...
export const createRbac = async (req, res) => {
  try {
    const { error, value } = createRbacSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateRbac = async (req, res) => {
  try {
    const { error, value } = updateRbacSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteRbac = async (req, res) => {
  try {
    const ctx = getRequestContext();
