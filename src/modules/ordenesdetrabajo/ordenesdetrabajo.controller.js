/**
 * Archivo: src/modules/ordenesdetrabajo/ordenesdetrabajo.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para ordenesdetrabajo.
 */
import service from "./ordenesdetrabajo.service.js";
import { createOrdenesdetrabajoSchema, updateOrdenesdetrabajoSchema } from "./ordenesdetrabajo.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listOrdenesdetrabajo = async (req, res) => {
  try {
...
export const getOrdenesdetrabajoById = async (req, res) => {
  try {
...
export const createOrdenesdetrabajo = async (req, res) => {
  try {
    const { error, value } = createOrdenesdetrabajoSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateOrdenesdetrabajo = async (req, res) => {
  try {
    const { error, value } = updateOrdenesdetrabajoSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteOrdenesdetrabajo = async (req, res) => {
  try {
    const ctx = getRequestContext();
