/**
 * Archivo: src/modules/dominios/dominios.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para dominios.
 */
import service from "./dominios.service.js";
import { createDominiosSchema, updateDominiosSchema } from "./dominios.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listDominios = async (req, res) => {
  try {
...
export const getDominiosById = async (req, res) => {
  try {
...
export const createDominios = async (req, res) => {
  try {
    const { error, value } = createDominiosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateDominios = async (req, res) => {
  try {
    const { error, value } = updateDominiosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteDominios = async (req, res) => {
  try {
    const ctx = getRequestContext();
