/**
 * Archivo: src/modules/tipodecuidado/tipodecuidado.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para tipodecuidado.
 */
import service from "./tipodecuidado.service.js";
import { createTipodecuidadoSchema, updateTipodecuidadoSchema } from "./tipodecuidado.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listTipodecuidado = async (req, res) => {
  try {
...
export const getTipodecuidadoById = async (req, res) => {
  try {
...
export const createTipodecuidado = async (req, res) => {
  try {
    const { error, value } = createTipodecuidadoSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateTipodecuidado = async (req, res) => {
  try {
    const { error, value } = updateTipodecuidadoSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteTipodecuidado = async (req, res) => {
  try {
    const ctx = getRequestContext();
