/**
 * Archivo: src/modules/ocupacion/ocupacion.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para ocupacion.
 */
import service from "./ocupacion.service.js";
import { createOcupacionSchema, updateOcupacionSchema } from "./ocupacion.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listOcupacion = async (req, res) => {
  try {
...
export const createOcupacion = async (req, res) => {
  try {
    const { error, value } = createOcupacionSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateOcupacion = async (req, res) => {
  try {
    const { error, value } = updateOcupacionSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
