/**
 * Archivo: src/modules/regimenhorario/regimenhorario.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para regimenhorario.
 */
import service from "./regimenhorario.service.js";
import { createRegimenhorarioSchema, updateRegimenhorarioSchema } from "./regimenhorario.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listRegimenhorario = async (req, res) => {
  try {
...
export const getRegimenhorarioById = async (req, res) => {
  try {
...
export const createRegimenhorario = async (req, res) => {
  try {
    const { error, value } = createRegimenhorarioSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateRegimenhorario = async (req, res) => {
  try {
    const { error, value } = updateRegimenhorarioSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteRegimenhorario = async (req, res) => {
  try {
    const ctx = getRequestContext();
