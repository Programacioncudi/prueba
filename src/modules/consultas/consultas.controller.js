/**
 * Archivo: src/modules/consultas/consultas.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para consultas.
 */
import service from "./consultas.service.js";
import { createConsultasSchema, updateConsultasSchema } from "./consultas.schema.js";
...
export const createConsultas = async (req, res) => {
  try {
    const { error, value } = createConsultasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateConsultas = async (req, res) => {
  try {
    const { error, value } = updateConsultasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
