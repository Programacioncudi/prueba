/**
 * Archivo: src/modules/usuarios_dominios/usuarios_dominios.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para usuarios_dominios.
 */
import service from "./usuarios_dominios.service.js";
import { createUsuariosDominiosSchema, updateUsuariosDominiosSchema } from "./usuarios_dominios.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listUsuariosDominios = async (req, res) => {
  try {
...
export const getUsuariosDominiosById = async (req, res) => {
  try {
...
export const createUsuariosDominios = async (req, res) => {
  try {
    const { error, value } = createUsuariosDominiosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateUsuariosDominios = async (req, res) => {
  try {
    const { error, value } = updateUsuariosDominiosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteUsuariosDominios = async (req, res) => {
  try {
    const ctx = getRequestContext();
