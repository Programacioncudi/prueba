/**
 * Archivo: src/modules/inconvenientesagentes/inconvenientesagentes.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para inconvenientesagentes.
 */
import service from "./inconvenientesagentes.service.js";
import { createInconvenientesagentesSchema, updateInconvenientesagentesSchema } from "./inconvenientesagentes.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listInconvenientesagentes = async (req, res) => {
  try {
...
export const getInconvenientesagentesById = async (req, res) => {
  try {
...
export const createInconvenientesagentes = async (req, res) => {
  try {
    const { error, value } = createInconvenientesagentesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateInconvenientesagentes = async (req, res) => {
  try {
    const { error, value } = updateInconvenientesagentesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteInconvenientesagentes = async (req, res) => {
  try {
    const ctx = getRequestContext();
