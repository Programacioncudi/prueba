/**
 * Archivo: src/modules/agentes/agentes.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para agentes.
 */
import service from "./agentes.service.js";
import { createAgentesSchema, updateAgentesSchema } from "./agentes.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listAgentes = async (req, res) => {
  try {
...
export const createAgentes = async (req, res) => {
  try {
    const { error, value } = createAgentesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateAgentes = async (req, res) => {
  try {
    const { error, value } = updateAgentesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
