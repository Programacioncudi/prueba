/**
 * Archivo: src/modules/tareasadquiridias/tareasadquiridias.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para tareasadquiridias.
 */
import service from "./tareasadquiridias.service.js";
import { createTareasadquiridiasSchema, updateTareasadquiridiasSchema } from "./tareasadquiridias.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listTareasadquiridias = async (req, res) => {
  try {
...
export const getTareasadquiridiasById = async (req, res) => {
  try {
...
export const createTareasadquiridias = async (req, res) => {
  try {
    const { error, value } = createTareasadquiridiasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateTareasadquiridias = async (req, res) => {
  try {
    const { error, value } = updateTareasadquiridiasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteTareasadquiridias = async (req, res) => {
  try {
    const ctx = getRequestContext();
