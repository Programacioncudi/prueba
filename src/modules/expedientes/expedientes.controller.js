/**
 * Archivo: src/modules/expedientes/expedientes.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para expedientes.
 */
import service from "./expedientes.service.js";
import { createExpedientesSchema, updateExpedientesSchema } from "./expedientes.schema.js";
...
export const getExpedientesById = async (req, res) => {
  try {
...
export const createExpedientes = async (req, res) => {
  try {
    const { error, value } = createExpedientesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateExpedientes = async (req, res) => {
  try {
    const { error, value } = updateExpedientesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteExpedientes = async (req, res) => {
  try {
    const ctx = getRequestContext();
