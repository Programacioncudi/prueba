/**
 * Archivo: src/modules/funciones/funciones.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para funciones.
 */
import service from "./funciones.service.js";
import { createFuncionesSchema, updateFuncionesSchema } from "./funciones.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listFunciones = async (req, res) => {
  try {
...
export const getFuncionesById = async (req, res) => {
  try {
...
export const createFunciones = async (req, res) => {
  try {
    const { error, value } = createFuncionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateFunciones = async (req, res) => {
  try {
    const { error, value } = updateFuncionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteFunciones = async (req, res) => {
  try {
    const ctx = getRequestContext();
