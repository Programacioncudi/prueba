/**
 * Archivo: src/modules/ccoodegdeba/ccoodegdeba.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para ccoodegdeba.
 */
import service from "./ccoodegdeba.service.js";
import { createCcoodegdebaSchema, updateCcoodegdebaSchema } from "./ccoodegdeba.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listCcoodegdeba = async (req, res) => {
  try {
...
export const getCcoodegdebaById = async (req, res) => {
  try {
...
export const createCcoodegdeba = async (req, res) => {
  try {
    const { error, value } = createCcoodegdebaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateCcoodegdeba = async (req, res) => {
  try {
    const { error, value } = updateCcoodegdebaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteCcoodegdeba = async (req, res) => {
  try {
    const ctx = getRequestContext();
