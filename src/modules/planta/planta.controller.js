/**
 * Archivo: src/modules/planta/planta.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para planta.
 */
import service from "./planta.service.js";
import { createPlantaSchema, updatePlantaSchema } from "./planta.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listPlanta = async (req, res) => {
  try {
...
export const createPlanta = async (req, res) => {
  try {
    const { error, value } = createPlantaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updatePlanta = async (req, res) => {
  try {
    const { error, value } = updatePlantaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
