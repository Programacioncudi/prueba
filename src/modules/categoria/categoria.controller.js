/**
 * Archivo: src/modules/categoria/categoria.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para categoria.
 */
import service from "./categoria.service.js";
import { createCategoriaSchema, updateCategoriaSchema } from "./categoria.schema.js";
...
export const createCategoria = async (req, res) => {
  try {
    const { error, value } = createCategoriaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateCategoria = async (req, res) => {
  try {
    const { error, value } = updateCategoriaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
