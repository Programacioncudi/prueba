/**
 * Archivo: src/modules/tipoderesolucion/tipoderesolucion.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para tipoderesolucion.
 */
import service from "./tipoderesolucion.service.js";
import { createTipoderesolucionSchema, updateTipoderesolucionSchema } from "./tipoderesolucion.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listTipoderesolucion = async (req, res) => {
  try {
...
export const getTipoderesolucionById = async (req, res) => {
  try {
...
export const createTipoderesolucion = async (req, res) => {
  try {
    const { error, value } = createTipoderesolucionSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateTipoderesolucion = async (req, res) => {
  try {
    const { error, value } = updateTipoderesolucionSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteTipoderesolucion = async (req, res) => {
  try {
    const ctx = getRequestContext();
