/**
 * Archivo: src/modules/roles_permisos/roles_permisos.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para roles_permisos.
 */
import service from "./roles_permisos.service.js";
import { createRolesPermisosSchema, updateRolesPermisosSchema } from "./roles_permisos.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listRolesPermisos = async (req, res) => {
  try {
...
export const getRolesPermisosById = async (req, res) => {
  try {
...
export const createRolesPermisos = async (req, res) => {
  try {
    const { error, value } = createRolesPermisosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateRolesPermisos = async (req, res) => {
  try {
    const { error, value } = updateRolesPermisosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteRolesPermisos = async (req, res) => {
  try {
    const ctx = getRequestContext();
