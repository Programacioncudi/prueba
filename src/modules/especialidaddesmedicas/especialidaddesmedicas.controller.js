/**
 * Archivo: src/modules/especialidaddesmedicas/especialidaddesmedicas.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para especialidaddesmedicas.
 */
import service from "./especialidaddesmedicas.service.js";
import { createEspecialidaddesmedicasSchema, updateEspecialidaddesmedicasSchema } from "./especialidaddesmedicas.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listEspecialidaddesmedicas = async (req, res) => {
  try {
...
export const getEspecialidaddesmedicasById = async (req, res) => {
  try {
...
export const createEspecialidaddesmedicas = async (req, res) => {
  try {
    const { error, value } = createEspecialidaddesmedicasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateEspecialidaddesmedicas = async (req, res) => {
  try {
    const { error, value } = updateEspecialidaddesmedicasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteEspecialidaddesmedicas = async (req, res) => {
  try {
    const ctx = getRequestContext();
