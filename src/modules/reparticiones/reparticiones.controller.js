/**
 * Archivo: src/modules/reparticiones/reparticiones.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para reparticiones.
 */
import service from "./reparticiones.service.js";
import { createReparticionesSchema, updateReparticionesSchema } from "./reparticiones.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listReparticiones = async (req, res) => {
  try {
...
export const getReparticionesById = async (req, res) => {
  try {
...
export const createReparticiones = async (req, res) => {
  try {
    const { error, value } = createReparticionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateReparticiones = async (req, res) => {
  try {
    const { error, value } = updateReparticionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteReparticiones = async (req, res) => {
  try {
    const ctx = getRequestContext();
