/**
 * Archivo: src/modules/personal/personal.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para personal.
 */
import service from "./personal.service.js";
...
export const createPersonal = async (req, res) => {
  try {
    const { error, value } = createPersonalSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updatePersonal = async (req, res) => {
  try {
    const { error, value } = updatePersonalSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
