/**
 * Archivo: src/modules/codigoa/codigoa.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para codigoa.
 */
import service from "./codigoa.service.js";
import { createCodigoaSchema, updateCodigoaSchema } from "./codigoa.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listCodigoa = async (req, res) => {
  try {
...
export const getCodigoaById = async (req, res) => {
  try {
...
export const createCodigoa = async (req, res) => {
  try {
    const { error, value } = createCodigoaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateCodigoa = async (req, res) => {
  try {
    const { error, value } = updateCodigoaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteCodigoa = async (req, res) => {
  try {
    const ctx = getRequestContext();
