/**
 * Archivo: src/modules/jefedeptos/jefedeptos.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para jefedeptos.
 */
import service from "./jefedeptos.service.js";
import { createJefedeptosSchema, updateJefedeptosSchema } from "./jefedeptos.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listJefedeptos = async (req, res) => {
  try {
...
export const getJefedeptosById = async (req, res) => {
  try {
...
export const createJefedeptos = async (req, res) => {
  try {
    const { error, value } = createJefedeptosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateJefedeptos = async (req, res) => {
  try {
    const { error, value } = updateJefedeptosSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteJefedeptos = async (req, res) => {
  try {
    const ctx = getRequestContext();
