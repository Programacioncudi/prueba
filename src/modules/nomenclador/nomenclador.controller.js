/**
 * Archivo: src/modules/nomenclador/nomenclador.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para nomenclador.
 */
import service from "./nomenclador.service.js";
import { createNomencladorSchema, updateNomencladorSchema } from "./nomenclador.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listNomenclador = async (req, res) => {
  try {
...
export const getNomencladorById = async (req, res) => {
  try {
...
export const createNomenclador = async (req, res) => {
  try {
    const { error, value } = createNomencladorSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateNomenclador = async (req, res) => {
  try {
    const { error, value } = updateNomencladorSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteNomenclador = async (req, res) => {
  try {
    const ctx = getRequestContext();
