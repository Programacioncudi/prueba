/**
 * Archivo: src/modules/roles/roles.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para roles.
 */
import service from "./roles.service.js";
import { createRolesSchema, updateRolesSchema } from "./roles.schema.js";
...
export const createRoles = async (req, res) => {
  try {
    const { error, value } = createRolesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateRoles = async (req, res) => {
  try {
    const { error, value } = updateRolesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
