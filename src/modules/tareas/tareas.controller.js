/**
 * Archivo: src/modules/tareas/tareas.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para tareas.
 */
import service from "./tareas.service.js";
import { createTareasSchema, updateTareasSchema } from "./tareas.schema.js";
...
export const createTareas = async (req, res) => {
  try {
    const { error, value } = createTareasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateTareas = async (req, res) => {
  try {
    const { error, value } = updateTareasSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
