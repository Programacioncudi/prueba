/**
 * Archivo: src/modules/localidades/localidades.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para localidades.
 */
import service from "./localidades.service.js";
import { createLocalidadesSchema, updateLocalidadesSchema } from "./localidades.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listLocalidades = async (req, res) => {
  try {
...
export const getLocalidadesById = async (req, res) => {
  try {
...
export const createLocalidades = async (req, res) => {
  try {
    const { error, value } = createLocalidadesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateLocalidades = async (req, res) => {
  try {
    const { error, value } = updateLocalidadesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteLocalidades = async (req, res) => {
  try {
    const ctx = getRequestContext();
