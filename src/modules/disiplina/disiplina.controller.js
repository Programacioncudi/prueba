/**
 * Archivo: src/modules/disiplina/disiplina.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para disiplina.
 */
import service from "./disiplina.service.js";
import { createDisiplinaSchema, updateDisiplinaSchema } from "./disiplina.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listDisiplina = async (req, res) => {
  try {
...
export const getDisiplinaById = async (req, res) => {
  try {
...
export const createDisiplina = async (req, res) => {
  try {
    const { error, value } = createDisiplinaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateDisiplina = async (req, res) => {
  try {
    const { error, value } = updateDisiplinaSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteDisiplina = async (req, res) => {
  try {
    const ctx = getRequestContext();
