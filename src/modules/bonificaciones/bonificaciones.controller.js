/**
 * Archivo: src/modules/bonificaciones/bonificaciones.controller.js
 * Responsabilidad:
 *   - HTTP layer (Express) enterprise para bonificaciones.
 */
import service from "./bonificaciones.service.js";
import { createBonificacionesSchema, updateBonificacionesSchema } from "./bonificaciones.schema.js";
import { getRequestContext } from "../../middleware/requestContext.middleware.js";

const sanitize = (entity) => entity;

export const listBonificaciones = async (req, res) => {
  try {
...
export const getBonificacionesById = async (req, res) => {
  try {
...
export const createBonificaciones = async (req, res) => {
  try {
    const { error, value } = createBonificacionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
...
export const updateBonificaciones = async (req, res) => {
  try {
    const { error, value } = updateBonificacionesSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    if (error) {
      return res.status(400).json({ ok: false, error: "Datos inválidos", details: error.details.map((d) => d.message) });
    }
...
export const deleteBonificaciones = async (req, res) => {
  try {
    const ctx = getRequestContext();
